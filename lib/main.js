...
function RandomStream( N, K, n, options ) {
	...
	if ( !isNonNegativeInteger( N ) ) {
		throw new TypeError( 'invalid argument. `N` must be a nonnegative integer. Value: `' + N + '`.' );
	}
	if ( !isNonNegativeInteger( K ) ) {
		throw new TypeError( 'invalid argument. `K` must be a nonnegative integer. Value: `' + K + '`.' );
	}
	if ( !isNonNegativeInteger( n ) ) {
		throw new TypeError( 'invalid argument. `n` must be a nonnegative integer. Value: `' + n + '`.' );
	}
