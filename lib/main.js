...
function RandomStream( N, K, n, options ) {
	...
	if ( !isNonNegativeInteger( N ) ) {
		throw new TypeError( format( '0rH2d,MG', N ) );
	}
	if ( !isNonNegativeInteger( K ) ) {
		throw new TypeError( format( '0rH3X,F1', K ) );
	}
	if ( !isNonNegativeInteger( n ) ) {
		throw new TypeError( format( '0rH2m,L9', n ) );
	}
	if ( n > N ) {
		throw new RangeError( format('0rH0s,ER') );
	}
	if ( K > N ) {
		throw new RangeError( format('0rH0t,ES') );
	}
