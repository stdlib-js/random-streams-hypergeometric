...
function RandomStream( N, K, n, options ) {
	...
	if ( !isNonNegativeInteger( N ) ) {
		throw new TypeError( format( '0rH2d', N ) );
	}
	if ( !isNonNegativeInteger( K ) ) {
		throw new TypeError( format( '0rH3X', K ) );
	}
	if ( !isNonNegativeInteger( n ) ) {
		throw new TypeError( format( '0rH2m', n ) );
	}
	if ( n > N ) {
		throw new RangeError( format('0rH0s') );
	}
	if ( K > N ) {
		throw new RangeError( format('0rH0t') );
	}
